...
class MainPage extends Component {
  ...
  componentWillReceiveProps({ country }) {
    ...
    if (countries !== undefined && countries.length > 0) {
      ...
      geographyObject.objects.ne_10m_admin_0_countries.geometries.forEach(element => {
        // Search country by name
        const foundCountry = countries.filter(e => e.name === element.properties.NAME);
        // Add data to geographyObject if country is in store
        if (foundCountry !== undefined && foundCountry.length > 0) {
          element.properties = Object.assign(element.properties, foundCountry[0]);
  ...
  // When user selects another country from drop-down
  handleSelectCountry = e => {
    const seekedCountry = e.target.value;
    const countries = geographyObject.objects.ne_10m_admin_0_countries.geometries;
    // Search country by name
    const foundCountry = countries.filter(e => seekedCountry === e.properties.NAME);
          ...
          this.state.isAdded ? (
            <div key={uuid()} className="d-inline">
              <ConfirmModal
                ...
                buttonText="Delete"
                title="Delete"
                message={`Are you sure you want to delete the data for ${this.state.selectedCountry.properties.name}?`}
                callBack={this.handleDelete.bind(this, this.state.selectedCountry.properties._id)}
              />
              <CountryModal
                key={uuid()}
        ...
        <Container className="container-fluid">
          <Row className="mt-5">
            <Col xs="12" md="2">
              ...
              <FormGroup>
                <Input type="select" name="selectedData" id="selectedData" onChange={e => this.handleSelectData(e)}>
                  {displayDataDropDownOptions}
                </Input>
              </FormGroup>
              {/* Countries List */}
              <CountriesList
                countries={geographyObject.objects.ne_10m_admin_0_countries.geometries}
                selectedData={this.state.selectedData}
              />
            </Col>
            {/* Europe Map*/}
            <CountriesMap selectedData={this.state.selectedData} callBack={this.handleClick} />
            <Col xs="12" md="2">
              ...
              <FormGroup>
                ...
                >
                  ...
                  {geographyObject.objects.ne_10m_admin_0_countries.geometries.map(({ properties, i }) =>
                    // Display only countries with store data
                    properties._id !== undefined ? <option key={properties._id}>{properties.name}</option> : null
                  )}
                </Input>
              </FormGroup>
              {/* Country Info*/}
              <CountryInfo selectedCountry={this.state.selectedCountry.properties} isAdded={this.state.isAdded} />
